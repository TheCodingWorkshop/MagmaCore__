...
use MagmaCore\UserManager\Controller\UserPreferenceController;
...
class UserPreferenceValidate extends AbstractDataRepositoryValidation
{
    ...
    /** @var string */
    protected const REDIRECT_BACK_TO = '/admin/user/index';
    ...
    {
        $this->rules = $rules;
        $this->rules->addObject(UserPreferenceController::class, $this);
    }
    ...
    public function validateBeforePersist(Collection $entityCollection, ?object $dataRepository = null): array
    {
        ...
        if (null !== $dataCollection) {
            $newCleanData = [
                'user_id' => $this->isSet('user_id', $dataCollection, $dataRepository),
                'enable_notification' => $this->isSet('user_id', $dataCollection, $dataRepository),
                'address' => $this->isSet('address', $dataCollection, $dataRepository),
                'language' => $this->isSet('language', $dataCollection, $dataRepository),
                'week_start_on' => $this->isSet('week_start_on', $dataCollection, $dataRepository),
            ];
    ...
    private function throwWarningIfNoChange(Collection $entityCollection, ?object $dataRepository = null)
    {
        if ($dataRepository !== null) {
            if ($entityCollection['user_id'] === $dataRepository->user_id) {
    ...
    public function validate(Collection $entityCollection, ?object $dataRepository = null): void
    {
        $this->doValidation(
            ...
            function ($key, $value, $entityCollection, $dataRepository) {
                if ($rules = $this->rules) {
                    return match ($key) {
                        'user_id' => $rules->addRule("required"),
